        ...
        for topic in topics_with_rss_links:
            try:
                if not hasattr(topic, 'rss_link') or not topic.rss_link:
                    logger.warning(f"Topic {topic.id if hasattr(topic, 'id') else 'unknown'} has no RSS link")
                    failed_fetches += 1
                    continue
                ...
                logger.info(f"Successfully fetched and added {articles_added} articles for {topic.rss_link}")
    ...
    try:
        ...
        for entry in rss_data.get("entries", []):
            try:
                ...
                published_at = datetime.strptime(entry.get("published_at"), "%Y-%m-%dT%H:%M:%S%z")
                current_date = datetime.utcnow().date()

                logger.info(f"Published at: {published_at.date()}, current date: {datetime.utcnow().date()}")
                ...
                if published_at.date() == current_date and articles_added < 10:
                    try:
                        ...
                        logger.warning(f"Failed to parse date '{entry.get('published_at')}': {str(date_e)}")
