    ...
    try:
        ...
        for broadcast in broadcasts:
            ...
            try:
                ...
                logger.info(f"Broadcast {broadcast.id} mp3 url saved to database successfully")

            except Exception as e:
                logger.error(f"Failed to convert broadcast {broadcast.id} to mp3: {str(e)}")
                continue
...
def _save_broadcast_mp3_url_to_database(db: Session, broadcast_id: int, broadcast_mp3_url: str):
    """Save broadcast mp3 url to database"""

    try:
        broadcasts_service = BroadcastsService(db)
        return broadcasts_service.save_broadcast_mp3_url(broadcast_id, broadcast_mp3_url)
    except Exception as e:
